...
class ChatLogic extends GetxController {
  ...
  final clickPlayNextSubject = rx.PublishSubject<MsgStreamEv<int>>();
  ...
  void playAudio(msgId, index) {
    clickPlayNextSubject.addSafely(
      MsgStreamEv<int>(msgId: msgId, value: index),
    );
  }
  ...
  void _sendMessage(Message message, {String? userId, String? groupId}) {
    ...
    _reset();
    String desc = message.contentType == MessageType.text
        ? message.content.toString()
        : StrRes.notificationTitle;
    String title = message.senderNickname.toString() + " " + desc;
    // 推送信息
  ...
  /// 重试发送
  void resendMsg(int index) async {
    /// TODO: 重发
    var confirm = await Get.dialog(CustomDialog(
      title: StrRes.confirmResend,
    ...
    // message.contentType = MessageType.fa;
    // messageList.refresh();
  }
  ...
  void call() {
    ...
    //       groupID: gid!, onTap: (int index, List<String> inviteeUserIDList) {});
    //   return;
    // }
    // IMWidget.openIMCallSheet(uid: uid!, name: name.value, icon: icon.value);
    IMWidget.showToast('Feature is not available on this version');
  }
  ...
  void copy(index) {
    ...
    if (msg.contentType == 101) {
      ...
      IMWidget.showToast(StrRes.notEnabledFunction);
    }
  }
  ...
  /// 发送下一首音频
  void playNextAudio(int cur, int index) async {
    bool isNext = false;
    do {
      if (cur - 1 < 0) {
        // IMWidget.showToast("没可播放的的语音了");
        return;
      }
      var message = indexOfMessage(cur - 1);

      if (message.contentType == MessageType.voice) {
        if (message.sendID == OpenIM.iMManager.uid) {
          // IMWidget.showToast("当前自动播放人不能是你自己");
          return;
        }
        isNext = true;
        this.playAudio(message.clientMsgID, cur - 1);
        break;
        IMWidget.showToast(message.clientMsgID.toString() +
            '消息id，消息内容' +
