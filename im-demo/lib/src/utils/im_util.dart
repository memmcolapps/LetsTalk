...
class IMUtil {
  ...
  static bool isNiMobile(String mobile) {
    RegExp exp = RegExp(r'^(00){0,1}\d{9,12}$');
    return exp.hasMatch(mobile);
  }
  ...
  static Future<File?> compressAndGetPic(
      File file, int? minWidth, int? minHeight, int? quality) async {
    var path = file.path;
  ...
  static Map<String, List<Message>> groupingMessage(List<Message> list) {
    ...
    list.forEach((message) {
      ...
      if (DateUtil.isToday(message.sendTime!)) {
        // 今天
        date = StrRes.isThisDay;
      } else if (DateUtil.isWeek(message.sendTime!)) {
        // 本周
        date = StrRes.isThisWeek;
      } else {
        // 按年月
